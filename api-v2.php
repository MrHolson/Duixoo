...
$error_code    = 0;
$error_message = '';
$type              = (!empty($_GET['type'])) ? Duix_Secure($_GET['type'], 0) : false;
$server_key        = (!empty($_POST['server_key'])) ? Duix_Secure($_POST['server_key'], 0) : false;
...
if ($server_key != $duix['config']['widnows_app_api_key']) {
    $response_data       = array(
        'api_status'     => '404',
        'errors'         => array(
            'error_id'   => '1',
...
if (!empty($_GET['access_token'])) {
    $get_user_id_from_access_token = Duix_ValidateAccessToken($_GET['access_token']);
    if (is_numeric($get_user_id_from_access_token) && $get_user_id_from_access_token > 0) {
        $duix['user'] = Duix_UserData($get_user_id_from_access_token);
        if (!empty($duix['user'])) {
            $duix['loggedin'] = true;
            if ($duix['user']['user_id'] < 0 || empty($duix['user']['user_id']) || !is_numeric($duix['user']['user_id']) || Duix_UserActive($duix['user']['username']) === false) {
               $duix['loggedin'] = false;
            }
        }
    }
...
if (!in_array($type, $pages_without_loggedin)) {
	if ($duix['loggedin'] == false && !empty($_GET['access_token'])) {
	    $error_code    = 2;
	    $error_message = 'Invalid or expired access_token';
	} else if ($duix['loggedin'] == false) {
		$error_code    = 2;
...
if ($duix['loggedin'] == true) {
    $duix['lang']      = Duix_LangsFromDB($duix['user']['language']);
}
