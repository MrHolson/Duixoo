<?php
// +------------------------------------------------------------------------+
// | @author Holson Mpangala (Hlone)
// | @author_url 1: https://www.holduix.com
// | @author_url 2: https://duixoo.com
// | @author_email: contact@holduix.com   
// +------------------------------------------------------------------------+
// | Duixoo Social Network
// | Copyright (c) 2020 Duixoo. All rights reserved.
// +------------------------------------------------------------------------+
...
if ($_GET['type'] == 'user_data') {
    $api_data = Duix_UserData(Duix_UserIdFromUsername($_GET['user']));
    if (empty($api_data)) {
        $json_error_data = array(
            'api_status' => 'failed',
            'api_version' => $api_version,
    ...
    $publisher_id = Duix_UserIdFromUsername($_GET['user']);
    ...
    foreach ($api_data as $post_data) {
        $json_data = array(
            ...
            'post_data' => array(
                ...
                'post_file' => Duix_GetMedia($post_data['postFile']),
                ...
                'post_likes' => Duix_CountLikes($post_data['post_id']),
                'post_wonders' => Duix_CountWonders($post_data['post_id'])
            ),
    ...
    if (!empty($_GET['keyword'])) {
        $filter['query'] = Duix_Secure($_GET['keyword']);
    }
    if (empty($_GET['keyword'])) {
        header("Content-type: application/json");
        echo json_encode(array(
    ...
    $api_data = Duix_GetSearchFilter($filter, $limit);
    if (empty($api_data)) {
        header("Content-type: application/json");
        echo json_encode(array(
            'api_status' => 'failed',
